...
def generateLoops_reversedRotate(N, r1, c1, r2, c2, withoutD):
	...
	L.append(tmp) # Group A is AntiClockwise
        
        
        
	# group B from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c1+1, c2), range(r1+1, r2)):
		L.append(generateSingleLoop(r1, c1, m, i, N)[::-1])

	
	# group C from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c2-1, c1, -1), range(r1+1, r2)):
		L.append(generateSingleLoop(r1, i, m, c2, N)[::-1])
	


	# group D from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c1+1, c2), range(r2-1, r1, -1)):
		L.append(generateSingleLoop(m, c1, r2, i, N)[::-1])

        # group E from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c2-1, c1, -1), range(r2-1, r1, -1)):
		L.append(generateSingleLoop(m, i, r2, c2, N)[::-1])
...
def generateLoops(N, r1, c1, r2, c2, withoutD):
	...
	# group B from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c1+1, c2), range(r1+1, r2)):
		L.append(generateSingleLoop(r1, c1, m, i, N))

	
	# group C from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c2-1, c1, -1), range(r1+1, r2)):
		L.append(generateSingleLoop(r1, i, m, c2, N))
	


	# group D from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c1+1, c2), range(r2-1, r1, -1)):
		L.append(generateSingleLoop(m, c1, r2, i, N))

        # group E from column c1+1 to c2-1 with c1 is fixed 
	for i,m in zip(range(c2-1, c1, -1), range(r2-1, r1, -1)):
		L.append(generateSingleLoop(m, i, r2, c2, N))
